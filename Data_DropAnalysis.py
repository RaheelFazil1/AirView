...
class DataDrops:
    def __init__(self, s_device):
        ...
        self.df_Signalling = com.readData("Signalling",
                                          ['Time', 'WorkOrderId', 'Device', 'RRC_NR5G_event', 'RRC_NR5G_pci',
        ...
        self.df_rc = com.readData("Event_NR",
                                  ['Time', 'WorkOrderId', 'NetworkModeId', 'BandId', 'CarrierId', 'SectorId', 'Device',
                                   'DeviceId', 'FileId', 'DimensionColumnsId', 'Epoch', 'MsgId', 'Latitude',
                                   'Longitude'],
                                  "Time", "asc")
        self.df_rc = self.df_rc.filter(self.df_rc.Device == gdevice)

        self.df_PHYDL_NR = com.readData("PHYDL_NR", ['Time', 'WorkOrderId', 'Device', 'PHYDL_NR5G_PCC_bler'], "Time",
                                        "asc")
    ...
    def an_window(self):
        Condition_Window = (self.df_event_NR.Event_CallEvent == '"Setup Start"') | \
                           (self.df_event_NR.Event_CallEvent == '"Call End"') | (
                               self.df_event_NR.Event_CallEvent.contains('Drop'))
        ...
        failswindow = df1.filter(df1.Event_CallEvent.contains('Drop'))
...
df_device3 = DataDrops('IPERF3_DL').f_final_df()
d1 = df_device3.toPandas()
df_device3.unpersist()

df_device4 = DataDrops('IPERF3_UL').f_final_df()
d2 = df_device4.toPandas()
df_device4.unpersist()
pdFrame = pd.concat([d1, d2])
# o_final_df.show()
# o_final_df.write.mode('overwrite').options(header='True').csv('/home/spark-master/Public/Raheel/final/drop')
...
com.output(pdFrame, "py_Data_DropAnalysis", cols)
